...
class ConfirmedRateAdapter :
    RecyclerView.Adapter<ConfirmedRateAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.apply {
            if (UIHelper.getWordsLength(current.Country.trim())!! > 1)
                ...
                place.textSize = 25f
            if (current.NewConfirmed != 0)
                newData.text = "+${UIHelper.withSuffix(current.NewConfirmed.toLong())}"
            no.text = UIHelper.withSuffix(current.TotalConfirmed.toLong())
            place.text = current.Country
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                cardLayout.setCardBackgroundColor(
                    context.getColor(R.color.confirmedCardColor)
                )
            } else {
                cardLayout.setCardBackgroundColor(
                    context.resources.getColor(R.color.confirmedCardColor)
                )
            }

...
class RecoveredRateAdapter :
    RecyclerView.Adapter<RecoveredRateAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.apply {
            if (UIHelper.getWordsLength(current.Country.trim())!! > 1)
                ...
                place.textSize = 25f
            if (current.NewRecovered != 0)
                newData.text = "+${UIHelper.withSuffix(current.NewRecovered.toLong())}"
            no.text = UIHelper.withSuffix(current.TotalRecovered.toLong())
            place.text = current.Country
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                cardLayout.setCardBackgroundColor(
                    context.getColor(R.color.recoveredCardColor)
                )
            } else {
                cardLayout.setCardBackgroundColor(
                    context.resources.getColor(R.color.recoveredCardColor)
                )
            }
